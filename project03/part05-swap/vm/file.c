...
file_backed_swap_in(struct page *page, void *kva)
{
	...
	file_read(aux->file, kva, aux->written_bytes);
...
file_backed_swap_out(struct page *page)
{
	...
	// 수정된 바 있으면 file_write 진행하고 dirty bit를 0으로
	if (pml4_is_dirty(thread_current()->pml4, page->va)) {
		file_write_at(aux->file, page->frame->kva, aux->written_bytes, aux->ofs);
		pml4_set_dirty(thread_current()->pml4, page->va, 0);
	}
	pml4_clear_page(thread_current()->pml4, page->va); // 페이지에 해당하는 pte의 present bit를 0으로
	palloc_free_page(page->frame->kva); // bitmap이 1인 것이 있는지 확인하고(잘못된 free page 요청인지 확인), bitmap 0으로 초기화, memset(pages, 0xcc, PGSIZE * page_cnt)까지 진행
