...
/* 
   이것은 최소한 int만큼 넓은 부호 없는 정수 유형이어야 합니다. 각 비트는 비트맵의 한 비트를 나타냅니다. 
   요소의 비트 0이 비트맵의 비트 K를 나타내는 경우 요소의 비트 1은 비트맵의 비트 K+1을 나타내는 식입니다. 
*/
...
*/
/* 외부에서 비트맵은 비트 배열입니다. 내부에서 비트 배열을 시뮬레이트하는 elem_type(위에 정의됨) 배열입니다. */
struct bitmap
{
	size_t bit_cnt;	 /* Number of bits. */ // 1 bit <--> 1 page 
...
{
	// // //printf("	### DIV_ROUND_UP(bit_cnt, ELEM_BITS): %d ###\n", DIV_ROUND_UP(bit_cnt, ELEM_BITS));
...
bitmap_create(size_t bit_cnt)
{
	...
	{
		b->bit_cnt = bit_cnt;
		b->bits = malloc(byte_cnt(bit_cnt)); // 가리키는 주소 공간에 952byte 할당
		// 1byte 당 8개의 page를 표현할 수 있으므로
		// 총 7560개를 표현하기 위해서는 952byte만 있으면 되는 것
		// 8 * 952 == 7616개 >> 7560개
...
bitmap_scan(const struct bitmap *b, size_t start, size_t cnt, bool value)
{
	...
	if (cnt <= b->bit_cnt)
	{
		...
		for (i = start; i <= last; i++)
			if (!bitmap_contains(b, i, cnt, !value)) // 952개 byte 중 i번째부터 cnt개의 byte를 확인하고 모든 bit가 0인지 확인
				return i;
	}
	return BITMAP_ERROR;
}
